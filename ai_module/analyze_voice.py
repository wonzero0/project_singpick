...
def load_basic_features(feature_dir):
    ...
    return pitch_avg, volume_avg, mfcc_mean, pitch_min, pitch_max, pitch_comfort_min, pitch_comfort_max
...
def analyze_voice(feature_dir):
    pitch_avg, volume_avg, _, user_min, user_max, comfort_min, comfort_max = load_basic_features(feature_dir)

    tempo_path = os.path.join(feature_dir, "tempo_bpm.npy")
    tempo_bpm = float(np.load(tempo_path).item()) if os.path.exists(tempo_path) else 0.0
